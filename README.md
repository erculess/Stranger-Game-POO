...
Para garantir que o código seja organizado e de fácil manutenção, a estrutura do projeto adotará uma arquitetura modular baseada nos conceitos de Programação Orientada a Objetos, separando claramente as responsabilidades de cada elemento na tela. O diretório raiz do jogo contará com um arquivo principal (como um main.py), que atuará como o motor da aplicação, gerenciando o loop principal do Pygame, a taxa de quadros por segundo (FPS) e o gerenciamento geral das telas (menu inicial, seleção de personagens e arena). Em paralelo, a lógica específica do jogo será dividida em scripts modulares independentes. Teremos um módulo dedicado exclusivamente aos lutadores (como um fighter.py), que conterá a classe base responsável por gerenciar a física de movimentação, o controle da barra de vida, as animações e o sistema de hitboxes de cada professor. A partir dessa base, haverá uma separação clara entre os comandos lidos pelo teclado para o jogador e os scripts de inteligência artificial que ditarão o comportamento do inimigo quando este for controlado pela máquina.

​Além da lógica focada nos personagens, a organização incluirá módulos para o controle do ambiente e do fluxo da partida. Um arquivo dedicado ao cenário (como um stage.py) ficará responsável por renderizar o ginásio do campus, delimitando as barreiras físicas da quadra e desenhando os alunos nas arquibancadas. O gerenciamento das colisões entre os ataques e os corpos dos lutadores, bem como a verificação das condições de vitória e derrota ao fim de cada round, será isolado em um gerenciador de combate central. Por fim, toda essa estrutura de código será acompanhada de um diretório de arquivos estáticos (geralmente chamado de assets), que será subdividido em pastas específicas para armazenar as imagens e animações (spritesheets dos professores e do fundo) e os efeitos sonoros do jogo, mantendo o ambiente de desenvolvimento limpo e facilitando o trabalho em equipe durante a construção.

## 12. Funcionalidades Mínimas

Para a primeira versão jogável do projeto (o Produto Mínimo Viável), a equipe deve focar exclusivamente em entregar a espinha dorsal de um jogo de luta, garantindo que o código rode sem falhas. Primeiramente, é obrigatório que o loop principal do Pygame esteja configurado e estável, sendo capaz de renderizar o fundo do ginásio e instanciar dois lutadores na tela (mesmo que, nesta fase inicial, utilizem formas geométricas ou sprites provisórios antes das artes finais dos professores). A movimentação bidimensional — andar, pular e agachar —, juntamente com o respeito absoluto às barreiras invisíveis nas bordas da quadra, deve estar totalmente operacional e respondendo aos comandos do teclado.

​No que diz respeito ao embate, a funcionalidade mais crítica da primeira versão é o sistema básico de colisões. O motor do jogo precisa reconhecer com precisão quando um ataque (hitbox) cruza com o corpo do adversário (hurtbox), aplicando imediatamente a dedução de dano na interface visual das barras de vida. Além disso, os movimentos primários de combate (soco, chute e a defesa simples) precisam funcionar fluidamente. Por fim, o jogo deve conter um fluxo de partida funcional, sendo capaz de monitorar o momento exato em que a barra de HP chega a zero, interromper a movimentação, declarar o nocaute e permitir que o jogador reinicie a luta ou feche o programa. Elementos complexos, como a inteligência artificial avançada do inimigo, os poderes especiais de todos os professores ou o menu de seleção completo, podem ser deixados para as versões seguintes, desde que dois personagens consigam trocar golpes básicos e finalizar um round corretamente na quadra do campus.

## 13. Melhorias Futuras

Após a consolidação da versão inicial com as mecânicas básicas de combate em funcionamento, o projeto possui um amplo espaço para expansão e refinamento. Uma das principais atualizações planejadas para as versões seguintes é a implementação de um Sistema de Pontuação inspirado nos arcades clássicos. Esse sistema não influenciará a força dos personagens, mas servirá para registrar o desempenho do jogador, concedendo pontos por ações específicas durante a luta, como a execução de combos extensos, vitórias perfeitas (sem sofrer dano) e bônus baseados no tempo restante no relógio ao final de cada round. Essa adição permitirá a criação de um ranking de maiores pontuações (High Scores) na tela inicial, incentivando a competição contínua. Além da pontuação, o jogo poderá evoluir com o aprimoramento da Inteligência Artificial, oferecendo diferentes níveis de dificuldade para o oponente controlado pela máquina. No aspecto audiovisual, o cenário poderá receber animações mais complexas e reativas para os alunos nas arquibancadas, além de um sistema robusto de áudio com efeitos sonoros para os impactos, narração de início de luta e reações da torcida. Por fim, para coroar a inspiração na franquia que baseia o projeto, há a possibilidade de desenvolver movimentos de finalização humorísticos e pedagógicos ao término das partidas, onde o professor vencedor aplica um golpe final cinematográfico característico de sua disciplina, como um gigantesco carimbo de "Reprovado".
